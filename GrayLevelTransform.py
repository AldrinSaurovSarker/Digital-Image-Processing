...
def print_image(image1, image2=None, name='Output Image'):
    if image2 is not None:
        output = cv2.hconcat([image1, image2])
...
while True:
    ...
    image = cv2.imread("images/human.jpg", 0)
    image_array = np.array(image)

    if choice == 1:
        result = negative(image_array)
        print_image(image, result, "Negative Transformation")
    elif choice == 2:
        result = log(image_array)
        print_image(image, result, "Log Transformation")
    elif choice == 3:
        result = inverse_log(image_array)
        print_image(image, result, "Inverse Log Transformation")
    elif choice == 4:
        result = nth_power(image_array)
        print_image(image, result, "nth Power Transformation")
    elif choice == 5:
        result = nth_root(image_array)
        print_image(image, result, "nth RootTransformation")
    elif choice == 6:
        result = gray_slice_with_bg(image_array)
        print_image(image, result, "Gray Slice With Background")
    elif choice == 7:
        result = gray_slice_without_bg(image_array)
        print_image(image, result, "Gray Slice With Background")
    elif choice == 8:
        result = bit_plane_slice(image_array)
        print_image(image, result, "Bit Plane Slicing")
    elif choice == 9:
        break
    else:
